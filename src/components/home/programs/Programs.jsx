        ...
        <div className="container w-full h-full text-gray-700 py-10">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 w-full h-full gap-4">
                ...
                {courses.filter(item => item.id === 1).map(({ id, img, program_name,program_subname,slider,intro,history_content,gallery }) => (
                        <div onClick={()=>{navigate(`/courses/${program_name}`,{state:{id,img,program_name,program_subname,slider,intro,history_content,gallery}});window.scrollTo(0,0)}} key={id} className="relative w-full h-full rounded-xl overflow-hidden group">
                            <div
