    ...
    return (
        ...
        <div className="container w-full h-full text-gray-700">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 w-full h-full gap-4">
                ...
                {courses.filter(item => item.id === 1).map(({ id, img, program_name,program_subname,slider,intro,history_content,gallery }) => {
                    console.log(program_name);
                        return <div onClick={()=>{navigate(`/courses/${program_name}`, {state:{id,img,program_name,program_subname,slider,intro,history_content,gallery}});window.scrollTo(0,0)}} key={id} className="relative w-full h-full rounded-xl overflow-hidden group cursor-pointer">
                            <div
                ...
                <div className="w-full grid grid-cols-1 lg:grid-cols-2 gap-4">
                    {courses.filter(item => item.id !== 1).map(({id, img, program_name,program_subname,slider,intro,history_content,gallery}) => (
                        <div onClick={()=>{navigate(`/courses/${program_name}`,{state:{id,img,program_name,program_subname,slider,intro,history_content,gallery}});window.scrollTo(0,0)}} key={id} className="relative w-full h-52 rounded-xl overflow-hidden group cursor-pointer">
                            <div
                                className="absolute inset-0 bg-center bg-cover bg-no-repeat transition-transform duration-700 group-hover:scale-110"
                                style={{ backgroundImage: `url(${img})` }}
                            ></div>
