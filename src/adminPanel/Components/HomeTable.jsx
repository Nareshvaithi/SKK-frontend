...
const HomeTable = () => {
  ...
  useEffect(()=>{
    const handleInitialValues=()=>{
       inputs.map(({value})=>{
        console.log(value)
        value=""
        setIntialValue({...intialValue,value})
       })
    }
    handleInitialValues()
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values,{ resetForm }) => {
        ...
        if(postValues){
            try {
                // await axios.post(`${API_URL}/${endPointCheck}`,values)
    ...
    <div className="py-10 relative h-full">
      ...
      {inputs.length > 0 ? (
        <div className="absolute inset-0 w-full h-screen bg-black/30 flex justify-center items-center overflow-scroll z-40">
          <div className={`w-1/2 bg-white p-5 rounded-2xl relative ${inputs.length>5 ? "mt-40": ""}`}>
            <form  onSubmit={formik.handleSubmit}  method="POST">
              ...
              <div className="flex items-center gap-5">
                ...
                <button type="button" className="bg-yellow-500 px-5 py-2 text-white font-semibold rounded-xl" onClick={() => formik.resetForm()}>
                  Reset
                </button>
              </div>
            </form>
